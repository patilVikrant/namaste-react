This is the fifth episode of Namaste React course by Akshay Saini.

In this episode we saw how react folder structure should be. First we created src folder in which we moved our App.js file then we created components folder in src folder, in which moved our various components such as Header, Body, etc. Then we learned about export and import of file in react.
There are two types of export and import one is default and other is named.
...
import {<Name of the variable>} from "<path>"

Then we learned about react hooks. In this episode we learned about useState hook which gives us super powerful react variable.
React hooks are javascript functions.

useState return an array of react variable and a function to update that variable. when the recat variable is updated react re-renders the component.

React uses reconciliation algorithm which is also known as React fiber.
React does efficient dom manipulation because it has a virtual dom. Virtual dom is representation of actual dom. It is a object. Whenever we update something a new virtual dom is created. Diff algorithm find out the difference between old virtual dom and new virtual dom. When the difference is found out react updates the DOM.
